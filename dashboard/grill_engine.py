...
def _json_object(text: str) -> dict[str, Any] | None:
    """Find the first valid JSON object in prose or a fenced response."""
...
def parse_interrogate_response(text: str) -> dict[str, Any]:
    """Normalize model JSON; an unusable response is a deterministic done fallback."""
    raw = _json_object(text)
    if raw is None or not isinstance(raw.get("done"), bool):
        return _empty_interrogate("engine returned an unusable response")
    done = raw["done"]
    if done:
        reason = _clean_text(raw.get("reason")) or "Nothing critical left."
        return {**_empty_interrogate(reason, source="model"), "settled_from_recommendation": bool(raw.get("settled_from_recommendation"))}
    question = _clean_text(raw.get("question"))
    recommended = _clean_text(raw.get("recommended"))
    category = _clean_text(raw.get("category"))
    if not question or not recommended or category not in _CATEGORIES:
        return _empty_interrogate("engine returned an unusable response")
    ...
    return {
        ...
        "options": [item for item in clean_options if item][:4],
        "category": category,
...
def parse_brief_response(text: str) -> str:
    ...
    if not isinstance(text, str):
        return ""
    clean = re.sub(r"^\s*```(?:markdown|md)?\s*|\s*```\s*$", "", text.strip(), flags=re.IGNORECASE)
    if "## Goal" not in clean or "## Directive" not in clean:
        return ""
...
def _default_llm(*, messages: list[dict[str, str]], temperature: float, max_tokens: int, timeout: float) -> tuple[str, str]:
    """Lazy Hermes adapter; import errors stay inside the request boundary."""
    from agent.auxiliary_client import call_llm, extract_content_or_reasoning
    route: dict[str, str] = {}
    # The configured grill route is normally OpenAI-compatible; request JSON mode for
    # the short interrogation call while preserving first-object parsing as the fallback.
    extra_body = {"response_format": {"type": "json_object"}} if max_tokens == 400 else None
    response = call_llm(
        task="grill", messages=messages, temperature=temperature, max_tokens=max_tokens,
        timeout=timeout, route_info=route, extra_body=extra_body,
    )
    provider = route.get("provider", "auto")
    model = route.get("model", "default")
    return extract_content_or_reasoning(response), f"{provider}/{model}"
...
def _invoke(llm: Callable[..., Any] | None, messages: list[dict[str, str]], *, max_tokens: int, timeout: float) -> tuple[str, str]:
    result = (llm or _default_llm)(messages=messages, temperature=0.2, max_tokens=max_tokens, timeout=timeout)
...
def interrogate(payload: Mapping[str, Any], llm: Callable[..., Any] | None = None) -> dict[str, Any]:
    ...
    try:
        if not text:
            ...
            raw, model = _invoke(llm, build_interrogate_messages(text, data.get("ladder"), data.get("cwd"), _clean_text(data.get("profile")) or None, force), max_tokens=400, timeout=_INTERROGATE_TIMEOUT)
...
def brief(payload: Mapping[str, Any], llm: Callable[..., Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raw, model = _invoke(llm, build_brief_messages(text, data.get("ladder"), data.get("cwd"), _clean_text(data.get("profile")) or None), max_tokens=900, timeout=_BRIEF_TIMEOUT)
